...
pub fn id<'id>() -> Id<'id> { PhantomData }
...
// range is a helper function that creates an Indices struct.
pub fn range<'id>(min: usize, max: usize) -> Indices<'id> { Indices::new(min, max) }
...
// The macro is used to create a function that takes an array and returns a tuple of the first and last elements.
// Example: 
...
mod tests {
    ...
    fn test_indexer_get() {
        ...
        // Get an Id
        let id: PhantomData<std::cell::Cell<&mut ()>> = id();

        let indexer: Indexer<'_, &[i32], i32> = Indexer { _id: id, arr: &arr, _marker: PhantomData };
        let index = Index::new(2);
        let result = indexer.get(index);
